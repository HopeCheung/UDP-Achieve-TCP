    ...
    while True:
        try:
            ...
            try:
                ...
                if len(keys) == 1:
                    print("Now send the last packet")
                    curr_num = keys[0]
                    packet = send_buffer[curr_num]
                    csum, rsum, curr_num, flag, data = extract_packet(packet)
                    packet = make_packet(curr_num, data, 2)
                    print(extract_packet(packet))
                    send_socket.sendto(packet, (address, port))
                    timer = datetime.datetime.now()
                else:
                    curr_num = min(keys)
                    print("Sending the data in send_buffer:{}".format(curr_num))
                    packet = send_buffer[curr_num]
                    send_socket.sendto(packet, (address, port))
                    timer = datetime.datetime.now()
